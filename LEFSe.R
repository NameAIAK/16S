...
#                         'Family','Genus',
#                         'Species')
last_7_cols <- df_split[, -(1:(ncol(df_split) - 6))] 
colnames(last_7_cols) <- c('Kingdom','Phylum',
                           'Class','Order',
                           'Family','Genus')
tax <- last_7_cols
...
df$sample_table$group %<>% factor(., levels = c("C","AB"))
str(df$sample_table)
# 
...
# 取LDA>=2
p_LDA <- lefse$plot_diff_bar(threshold = 2,#设定LDA SCORE显示阈值
                    width = 0.8,#柱子宽度
                    group_order = c( "AB", "C"),
...
write.table(lefse$res_diff, file = "HYXM_16S.lefse.res_diff.csv", sep = ",", row.names = FALSE, quote = FALSE, na = "")
...
# )
# 默认取LDA前20
p_abund <- lefse$plot_diff_abund(group_order = c("AB", "C"),
                                 use_number = 1:9,
                      add_sig = T,
...
write.table(random_forest_all$res_diff, file = "HYXM_16S.rf.res_diff.csv", sep = ",", row.names = FALSE, quote = FALSE, na = "")
...
ggsave(
  plot = p_rfgini_all,
  "HYXM_16S.p_rfgini.all.pdf",
  height = 8,
  width = 10
)
# 使用p1中相同的分类群绘制丰度图

p_rfabun_all <- random_forest_all$plot_diff_abund(use_number = 1:20,group_order = c("AB", "C"),
                                    add_sig = T,
                                    color_values = RColorBrewer::brewer.pal(8, "Set1"))
ggsave(
  plot = p_rfabun_all,
  "HYXM_16S.p_rfabun1-20.all.pdf",
  height = 8,
  width = 10
)
# 拼图
...
ggsave(
  plot = p_rfgini,
  "HYXM_16S.p_rfgini.genus.pdf",
  height = 6,
  width = 12
)
# 使用p1中相同的分类群绘制丰度图

p_rfabun <- random_forest$plot_diff_abund(use_number = 1:20,group_order = c("AB", "C"),
                                          add_sig = T,
                                          color_values = RColorBrewer::brewer.pal(8, "Set1"))
ggsave(
  plot = p_rfabun,
  "HYXM_16S.p_rfabun1-20.genus.pdf",
  height = 6,
  width = 8
)
