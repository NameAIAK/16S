...
PMB <- fread("../PCancerMb/P-6.csv")
NMB <- fread("../PCancerMb/N-6.csv")
...
# P_id <- id[,c("mb_id_P",'group')]
# N_id <- id[,c("mb_id_N",'group')]
...
# rownames(meta_id) <- meta_id$sample
map2 <- meta_id[,'group']
map2 <- as.data.frame(map2)
rownames(map2) <- meta_id$sample
...
# taxonomy <- sapply(strsplit(colnames(df1), ";g__"), function(x) x[length(x)])
# taxonomy <- gsub(";s__", ";", taxonomy)
# colnames(df1) <- taxonomy
#####metabolite#####
...
# 使用fread()读取CSV文件，假设第一行是列名  

PMB <- as.data.frame(PMB)
colnames(PMB)[1] <- 'name'

mb_pos <- PMB[,c('name',P_id$mb_id_P)]
# sort(colnames(PMB))
...
mb_neg <- NMB[,c('name',N_id$mb_id_N)]
...
write.csv(p.out,"R.pos6.pout.csv",row.names = F)
write.csv(r.out,"R.pos6.rout.csv",row.names = F)

df_pr <-melt(result_r,value.name="cor")
df_pr$pvalue <-as.vector(result_p)  #宽格式转长格式
df_filt <- subset(df_pr,abs(df_pr$cor)>0.4&df_pr$pvalue<0.05)#筛选
# df_filt <- subset(df_pr,abs(df_pr$cor)>0.75)#筛选
# df_filt <- subset(df_pr,df_pr$pvalue<0.05)#筛选
write.csv(df_pr,"R.pos6.melt_p_r.csv",row.names = F)#输出长格式
write.csv(df_filt,"R.pos6.melt_p005_r06.csv",row.names = F)
...
# pheatmap(result_r,file = "HYXM_16S.abun_pos_cor.heatmap.pdf",width=16,height=8,scale = "row",cluster_row = T, cluster_col = T, border=NA,display_numbers = result_p, number_color = "black")#根据自己的喜好绘制热图
...
pheatmap(filt_r,file = "R.HYXM_16S.abun_pos6_cor_filt.r6p005.heatmap.pdf",width=20,height=3,cluster_row = T, cluster_col = T, border=NA,display_numbers = filt_p, number_color = "black")#根据自己的喜好绘制热图
...
p.out<-cbind(rownames(result_p),result_p) #输出p值
r.out<-cbind(rownames(result_r),result_r) #输出cor值
write.csv(p.out,"R.neg6.pout.csv",row.names = F)
write.csv(r.out,"R.neg6.rout.csv",row.names = F)

df_pr <-melt(result_r,value.name="cor")
df_pr$pvalue <-as.vector(result_p)  #宽格式转长格式
df_filt <- subset(df_pr,abs(df_pr$cor)>0.4&df_pr$pvalue<0.05)#筛选
write.csv(df_pr,"R.neg6.melt_p_r.csv",row.names = F)#输出长格式
write.csv(df_filt,"R.neg6.melt_p005_r06.csv",row.names = F)
...
filt_r <- result_r[c(unique(df_filt$Var1)),c(unique(df_filt$Var2))]
filt_p <- result_p[c(unique(df_filt$Var1)),c(unique(df_filt$Var2))]
pheatmap(filt_r,file = "R.HYXM_16S.abun_neg6_cor.r06p005.heatmap.pdf",width=18,height=3,cluster_row = T, cluster_col = T, border=NA,display_numbers = filt_p, number_color = "black")#根据自己的喜好绘制热图


#P2,N2,P4,N4,P5,N5,P6,N6,P7,N7没有结果
...
write_graph(g, 'R.HYXM_16S.network_pos6.graphml', format = 'graphml')
...
plot(g)
#graphml 格式，可使用 gephi 软件打开并进行可视化编辑
write_graph(g, 'R.HYXM_16S.network_neg6.graphml', format = 'graphml')
...
pheatmap(filt_r,file = "HYXM_16S.abun_pos_neg_cor_filt.r6p005.all.heatmap.pdf",width=12,height=18,cluster_row = T, cluster_col = T, border=NA,display_numbers = filt_p, number_color = "black")#根据自己的喜好绘制热图
####相关性网络neg####
##计算微生物类群丰度和代谢物鉴定强度的相关系数
library(Hmisc)
MAG<-df1
...
plot(g)
#graphml 格式，可使用 gephi 软件打开并进行可视化编辑
write_graph(g, 'HYXM_16S.network_pos_neg.all.graphml', format = 'graphml')
...
ggsave(
  plot = p1,
  "HYXM_16S.O2PLS_pos_species.pdf",
  height = 8,
  width = 10
)        
...
ggsave(
  plot = p2,
  "HYXM_16S.O2PLS_pos_metabolite.pdf",
  height = 8,
  width = 10
) 
...
ggsave(
  plot = p3,
  "HYXM_16S.O2PLS_neg_species.pdf",
  height = 8,
  width = 10
)        
yj<- loadings(modelfit, "Yjoint", 1:2) %>% abs %>% rowSums
...
ggsave(
  plot = p4,
  "HYXM_16S.O2PLS_neg_metabolite.pdf",
  height = 8,
  width = 10
) 
